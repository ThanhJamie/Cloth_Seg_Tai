...
def convert_label_to_4D(label):
    label1 = (label == 1).float().unsqueeze(0)
    label2 = (label == 2).float().unsqueeze(0)
    label3 = (label == 3).float().unsqueeze(0)
    label4 = (label == 4).float().unsqueeze(0)
...
def training_loop(opt):
    ...
    u_net = U2NETP(in_ch=3, out_ch=47)
    ...
    weights = np.array([1, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5], dtype=np.float32)
    ...
    for itr in pbar:
        ...
        if local_rank == 0:
            ...
            if itr % opt.image_log_freq == 0:
                ...
                # print(torch.min(d0) , "--" , torch.max(d0))
                visuals = [[image, torch.unsqueeze(label, dim=1) * 255/46, d0 * 255/46]]
