...
def options_printing_saving(opt):
    ...
    for key, value in option_dict.items(): # Printing options
        print(key, value)
...
def training_loop(opt):
    ...
    if opt.distributed: # If Multi GPU Training is enabled
        if u_net is not None:
...
if __name__ == "__main__":
    ...
    try:
        if opt.distributed: # If Multi Cpu Training is enabled
